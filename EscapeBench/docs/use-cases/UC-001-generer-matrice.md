...
1. Le chercheur demande la génération d'une Matrix en précisant les tailles (bornes et pas), la présence ou non d'un champ pointeur, les dispositions de type, les LifetimeProfile, les modes de passage, le nombre d'instances produites par opération et les Probe (genre et paramètre), ou demande la matrice de référence.
2. Le système valide les paramètres : tailles multiples de 8 dans [8, 4096], dispositions, profils et genres de Probe appartenant aux listes du modèle d'entités, au moins un mode de passage, nombre d'instances par opération ≥ 1, paramètres de Probe > 0 et, pour les sondes qui parcourent la mémoire, multiples de 64 octets couvrant au moins deux nœuds.
...
4. Le système génère, pour chaque combinaison taille × présence de pointeur × disposition, un TypeSpec, pour chaque TypeSpec × LifetimeProfile × nombre d'instances par opération, une Cell par mode de passage, et une Probe par genre × paramètre demandé, chacune avec son fichier source sous `matrices/<matrixId>/`.
