...
- **Validité externe.** Micro-benchmarks sur des types synthétiques, une seule machine, une seule soirée, Windows sur amd64 et Go 1.27.0, alors que le livre se réfère à Go 1.25. L'analyse d'échappement varie d'une version du compilateur à l'autre (Shahsavan, 2026, p. 242) : les résultats sont datés par construction. Depuis le retrait de la CI (D-53), seule la suite de tests a été rejouée hors du poste, sous WSL2 sur le même processeur, sans rien mesurer; le rejeu des campagnes sous Linux reste à faire (D-62) : [`Doc/VERIFICATION-HORS-POSTE.md`](Doc/VERIFICATION-HORS-POSTE.md).
...
| [`Doc/DECISION.md`](Doc/DECISION.md) | Journal des décisions D-01 à D-65 (D-56 attend la revue des cas d'utilisation par un tiers, D-62 le rejeu des campagnes sous Linux) : écarts assumés, conception du harnais, statistiques, clôture, correctifs de l'audit, dépôt final, implantation de l'évaluation |
...
| [`Doc/DECISION_LeakLab.md`](Doc/DECISION_LeakLab.md) | Journal des décisions de LeakLab, D-01 à D-19 (D-18 attend la revue des cas d'utilisation) : gel, architecture, oracle, détecteurs, décisions prises après la première campagne |
...
Les métadonnées de citation sont dans [`CITATION.cff`](CITATION.cff), que GitHub affiche sous « Cite this repository ». L'état évalué le 2026-09-15 porte l'étiquette `v1.0` (historique purgé, D-55); l'état qui implante l'évaluation porte `v1.1`. Aucun DOI n'est encore attribué : la release GitHub et son archivage par Zenodo restent à faire par le chercheur, et le DOI sera alors reporté ici et dans `CITATION.cff`.
