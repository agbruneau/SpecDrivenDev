...
Une exécution vérifie le code et les verdicts archivés ; elle ne mesure rien. Les campagnes de rejeu sous Linux (lot 8, actions 2 et 3) sont produites par le binaire, jamais écrites par un agent, et ont leurs propres rapports sous `Campagnes/`. Celles du 2026-09-23 ont été lancées par l'agent, à la demande explicite du chercheur (D-62).
...
| 2026-09-23 | `3c2dfda` (copie par `git archive`, sur le disque de la distribution) | Ubuntu 24.04.4 LTS sous WSL2, même poste | go1.27.0 linux/amd64 | Campagnes (lancées par l'agent à la demande du chercheur) : LeakLab `run` puis `verdict`; EscapeBench, les deux séquences de `LANCEMENT.md` §5, en série, puis `compare` et `verdict` | `R-2026-09-23-1` (9 min 27), `C-2026-09-23-1` (1 h 02), `C-2026-09-23-2` (10 min), aucun sujet en échec. Quatre verdicts d'EscapeBench et un de LeakLab changent (D-62, D-20). Suite des deux bancs ensuite, sous Windows : 84 verdicts rejoués sur 14 campagnes. |
...
## Rejeu des campagnes sous Linux (fait le 2026-09-23 ; procédure conservée pour un rejeu futur)
...
1. LeakLab (environ 9 min) : `go run ./cmd/leaklab run`, puis `go run ./cmd/leaklab verdict -run <runId>`. Comparer à `R-2026-09-13-2` cellule par cellule.
...
3. Rapports `Campagnes/RAPPORT-CAMPAGNE_R-<date>-linux.md` et `Campagnes/RAPPORT-CAMPAGNE_C-<date>-linux.md` : treize verdicts contre treize, quatorze contre quatorze, écart médian des cellules. Les décisions D-62 (EscapeBench) et D-20 (LeakLab) consignent le rejeu du 2026-09-23 : [`RAPPORT-CAMPAGNE_C-2026-09-23-linux.md`](../Campagnes/RAPPORT-CAMPAGNE_C-2026-09-23-linux.md), [`RAPPORT-CAMPAGNE_R-2026-09-23-linux.md`](../Campagnes/RAPPORT-CAMPAGNE_R-2026-09-23-linux.md). Les verdicts qui changent s'ajoutent par plateforme; ils ne remplacent jamais la série de référence.
