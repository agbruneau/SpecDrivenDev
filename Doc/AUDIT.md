...
**Date :** 2026-09-12 · **Portée :** `EscapeBench/` (74 fichiers Go, 16 402 lignes), gabarits du harnais, hooks, `Makefile`, CI, et la conformité du code aux cinq cas d'utilisation et au catalogue d'exigences. **État du dépôt audité :** `main`, commit `05a08b4`, arbre propre. **Toolchain :** `go1.27.0 windows/amd64`.
...
*Ajouté le 2026-09-22 (lot 10 du plan d'implantation de l'évaluation, E-26). Aucun constat n'est retiré ni reformulé.*

[Résultat](#résultat) · [Méthode](#méthode) et son [erratum du 2026-09-22](#erratum-du-2026-09-22--décomptes-de-la-méthode) · [Ce qui tient](#ce-qui-tient) · [Lecture des constats](#lecture-des-constats) · [État d'implantation au 2026-09-12](#état-dimplantation-au-2026-09-12) · [Planification des correctifs](#planification-dexécution-des-correctifs) · [Inventaire](#inventaire-des-constats) : [bloquants](#bloquants-3), [majeurs](#majeurs-12), [mineurs](#mineurs-53), [suggestions](#suggestions-35), [écartés](#constats-écartés-par-la-vérification-4), [confirmés à la contre-vérification](#constats-confirmés-lors-de-la-contre-vérification-8-majeurs), [sans vérification contradictoire complète](#constats-sans-vérification-contradictoire-complète-75).

| Section de l'inventaire | Entrées | Gravité | État |
|---|---|---|---|
...
### Erratum du 2026-09-22 : décomptes de la méthode

Le paragraphe « Méthode » ci-dessus reste tel que publié ; cet erratum le corrige sans le réécrire. Source : le fichier d'exécution du workflow `audit-escapebench` (`wf_e30c209a-4c3`, état final du 2026-09-12 à 12 h 19 UTC), décrit dans [`Revue/PROMPTS-REVUES_2026-09-10.md`](../Revue/PROMPTS-REVUES_2026-09-10.md), section 6, et dans [`QR2-MESURES.md`](QR2-MESURES.md). Il donne 290 constats bruts, 187 après dédoublonnage, 119 confirmés et 68 réfutés. 189 des 581 agents ont échoué sur les limites d'usage du compte (188 vérificateurs et le critique de complétude), et la seconde ronde prévue n'a pas eu lieu. Ce document publie 190 items : 102 confirmés, 4 rejetés, 84 non vérifiés.

**Vérifié, en recoupant un à un les 187 identifiants du fichier d'exécution et ceux de l'inventaire** :

1. **190 entrées pour 187 constats.** Trois constats sont inscrits deux fois, parmi les confirmés et parmi les 75 : A-128 et A-133 (Mineurs), A-134 (Suggestions). L'inventaire ne contient aucun constat absent du fichier. Les dix-neuf autres identifiants cités dans ce document (A-022, A-031, etc.) sont des constats absorbés par la fusion.
2. **102 contre 119 confirmés.** Les 102 confirmés publiés sont exactement les 102 constats confirmés par le fichier et rangés sous Bloquants (2, avant la promotion de A-261), Majeurs, Mineurs et Suggestions. Les 17 autres confirmés du fichier ont été publiés comme non vérifiés : A-156, A-185 et A-187, repris ensuite dans la contre-vérification, et 14 des 75 (A-129, A-138, A-145, A-147, A-159, A-160, A-161, A-163, A-188, A-191, A-192, A-193, A-194 et A-195). Il faut y ajouter les trois doublons du point 1, eux aussi confirmés.
...
Aucun constat n'est retiré. L'état des 75 au 2026-09-22 est porté dans leur tableau.
...
État vérifié le 2026-09-22 contre le code actuel d'EscapeBench (lot 10 du plan d'implantation de l'évaluation, E-26). Trois valeurs : *implanté au fil de l'eau*, *ouvert*, *sans objet depuis D-53*. L'indice est ce qui fonde l'état : fichier, fonction, test ou commit. « En partie » signale un constat dont une moitié est faite : il reste ouvert. Aucun des 75 ne portait sur la CI : aucun n'est sans objet depuis D-53. La colonne « Votes » est celle de la publication du 2026-09-12 ; l'erratum de la section Méthode dit pourquoi elle ne concorde pas avec le fichier d'exécution.

| Constat | Sévérité annoncée | Emplacement | Énoncé | Votes | État au 2026-09-22 | Indice |
|---|---|---|---|---|---|---|
| A-147 | Mineur | `cmd/escapebench/main.go:67` | Après le premier SIGINT, tous les signaux suivants sont avalés : un second Ctrl+C ne peut pas forcer l'arrêt | 0/3 | implanté au fil de l'eau | `cmd/escapebench/main.go`, `interruptContext` : `signal.Stop` après le premier signal, le second retrouve le comportement du système (commentaire A-147, commit `9e81f6d`). |
| A-268 | Mineur | `cmd/escapebench/main.go:113` | L'étape 7 et A2 sont tautologiques en production : l'empreinte du harnais est calculée une fois sur embed.FS et ne peut pas changer pendant la… | 0/3 | ouvert | `main.go` : `HarnessDigest` rend `renderer.Digest()`, calculé sur `embed.FS` ; l'étape 7 de `measure` compare toujours deux valeurs du même binaire. |
| A-271 | Mineur | `cmd/escapebench/main.go:249` | L'étape 4 (afficher identifiant, provenance et décomptes) n'a lieu qu'après l'étape 8 : rien n'est affiché pendant la campagne | 0/3 | ouvert | `runCampaign` n'imprime `RenderCampaign` qu'au retour de `svc.Run`, donc après l'étape 8. |
| A-254 | Mineur | `docs/entity-model.md:77` | Table Probe cassée : un paragraphe est inséré entre deux lignes et la ligne sourceFile est orpheline | 0/3 | ouvert | `docs/entity-model.md`, section Probe : le paragraphe sur `POINTER_CHASE` coupe encore le tableau avant la ligne `sourceFile`. |
| A-253 | Mineur | `docs/entity-model.md:85` | Matrix.parameters n'énumère pas les dimensions layouts, repeats, payloads, replicates que le code et le JSON portent — spec à mettre à jour d'abord | 0/3 | ouvert | `Matrix.parameters` énumère encore « tailles, champ pointeur, profils, modes, Probe » ; ni `layouts`, ni `repeats`, ni `payloads`, ni `replicates`. |
| A-252 | Mineur | `docs/use-cases/UC-001-generer-matrice.md:26` | Le scénario principal de UC-001 ne nomme ni la dimension de réplicat ni deux des quatre sauts de génération — spec à mettre à jour d'abord | 0/3 | ouvert | UC-001, étapes 1 et 4 : la dimension de réplicat n'y figure pas ; seule la note de revue sur C-009 la nomme. |
| A-284 | Mineur | `docs/use-cases/UC-004-comparer-valeur-pointeur.md:26` | UC-004 étape 5 définit le point de bascule par couple (profil, champ pointeur) ; le code le calcule par triplet avec la disposition | 0/3 | ouvert | UC-004, étape 5 : « couple (LifetimeProfile, présence d'un champ pointeur) » ; `models.TippingKey` porte aussi `Layout`. |
| A-133 | Mineur | `internal/adapters/cli/cli_test.go:16` | Aucun test hors internal/service n'est nommé TestUC###_… : 0 sur cmd, adapters, models et harness (205 fonctions de test) | 2/3 | ouvert | En partie : 27 tests `TestUC###_` hors `internal/service` (aucun à l'audit), sur environ 190. `CLAUDE.md` n'exige plus de renommer les tests existants (E-32). |
| A-172 | Mineur | `internal/adapters/escape/escape.go:69` | CompilerReason conserve le séparateur de chemin du système : un rapport d'échappement n'est pas reproductible octet pour octet entre Windows et Linux | 0/3 | ouvert | `escape.go` : `CompilerReason` reçoit `diag.Raw`, chemin tel que le compilateur l'écrit, séparateur compris. |
| A-161 | Mineur | `internal/adapters/gotool/gotool.go:119` | La sortie brute de `go build`/`go test` est consignée dans results/ et peut contenir le chemin absolu du poste | 0/3 | ouvert | `gotool.go`, `Run` : la sortie brute est consignée sans filtrage des chemins ; depuis D-60 elle est archivée en entier. |
| A-196 | Mineur | `internal/adapters/gotool/gotool_test.go:268` | TestExecRunner dépend du binaire `go` sur le PATH dans la suite unitaire | 0/3 | ouvert | `TestExecRunner` exécute `go env GOARCH` dans la suite unitaire. |
| A-128 | Mineur | `internal/adapters/gotool/quietude_test.go:96` | time.Sleep dans un test, aucun usage de testing/synctest, parce que Toolchain.Run lit l'horloge réelle (time.Now/time.Since) au lieu de l'horloge… | 2/3 | implanté au fil de l'eau | `gotool/quietude_test.go` : le test tourne dans `synctest.Test` ; le `time.Sleep` restant avance l'horloge de la bulle (règle amendée, D-59). |
| A-163 | Mineur | `internal/adapters/specs/specs.go:45` | Une ligne d'hypothèse à plus de cinq cellules ou un identifiant en double passent silencieusement dans l'empreinte des critères | 0/3 | ouvert | `specs.go`, `Load` : seul un nombre de cellules inférieur à cinq est refusé ; ni l'excédent ni l'identifiant en double ne sont détectés. |
| A-159 | Mineur | `internal/adapters/store/store.go:203` | Identifiants de matrice et de campagne issus des drapeaux joints tels quels aux chemins : `..` sort de matrices/ et de results/ | 0/3 | implanté au fil de l'eau | `store.go` (commentaire A-159) et `TestNFR4_IdentifiantNeSortPasDuPerimetre` (commit `6b73dc7`). |
| A-162 | Mineur | `internal/adapters/store/store.go:252` | Les lecteurs (Load, LoadCampaign, LoadMeasurements) ne valident pas ce qu'ils décodent, contrairement aux écrivains | 0/3 | ouvert | `store.go` : `Load` et `LoadCampaign` rendent `dto.toModel()` sans `Validate`. |
| A-164 | Mineur | `internal/adapters/store/store.go:544` | Tous les fichiers écrits par writeFileAtomic naissent en mode 0600 sur Linux/macOS | 0/3 | implanté au fil de l'eau | `store.go` (commentaire A-164) et `TestNFR4_DroitsDesFichiersDeResultats` (commit `6b73dc7`). |
| A-195 | Mineur | `internal/adapters/store/store_test.go:351` | store_test teste LatestVerdictReport (sans appelant en production) et pas VerdictReports (utilisé par le tableau de bord, C-009) | 0/3 | implanté au fil de l'eau | `store_test.go` éprouve maintenant `VerdictReports` (commentaire A-195, commit `3bd223b`). |
| A-129 | Mineur | `internal/adapters/system/quietude_test.go:91` | TestC010_SondeReelle est un test temporel sur horloge réelle et compteurs système, sans synctest, à résultat dépendant de la charge de la machine | 1/3 | implanté au fil de l'eau | `system/quietude_test.go` : exemption de `synctest` écrite en commentaire, conformément à la règle amendée (D-59). |
| A-188 | Mineur | `internal/harness/harness_test.go:21` | Aucun test du harnais ne détecte une empreinte constante : Digest() figé à 64 « a » passe toute la suite | 0/3 | ouvert | `TestDigestStableEtNonVide` et `TestDigestChangeAvecLesGabarits` passent encore avec une constante de 64 caractères autre que l'empreinte d'avant C-008. |
| A-247 | Mineur | `internal/models/matrix.go:238` | La règle « au moins deux nœuds » des sondes de parcours n'est pas appliquée à l'étape 2 mais au rendu | 0/3 | ouvert | `Matrix.Validate` (`models/matrix.go`) n'exige que le multiple de 64 ; la règle des deux nœuds reste dans `harness.go`, au rendu. |
| A-145 | Mineur | `internal/service/campaign.go:195` | L'échec de ReleaseLock est avalé : un verrou orphelin bloque silencieusement UC-001 et UC-003 | 0/3 | implanté au fil de l'eau | `campaign.go`, `releaseLock` joint l'échec de libération à l'erreur rendue (commentaire A-145, commit `6b73dc7`). |
| A-151 | Mineur | `internal/service/campaign.go:227` | Sur une reprise, Duration couvre l'interruption entière (StartedAt d'origine à la fin de la reprise) | 0/3 | ouvert | `measure` calcule toujours `finishedAt.Sub(campaign.StartedAt)`, reprise comprise. La ronde finale du workflow l'avait réfuté à deux voix contre une (erratum ci-dessus). |
| A-269 | Mineur | `internal/service/campaign.go:243` | Comportement absent du UC : la campagne est passée ABORTED avec la raison « aucun sujet mesuré » | 0/3 | ouvert | UC-003 ne décrit pas l'abandon « aucun sujet mesuré » ; le code (`ErrNoSubjectMeasured`) et `TestUC003_AucunSujetMesure` le font. |
| A-193 | Mineur | `internal/service/campaign_test.go:100` | Étape 5 « chaque Cell puis chaque Probe » : seul order[0] est vérifié | 0/3 | ouvert | `TestUC003_MainFlow` ne vérifie que `order[0]`. |
| A-194 | Mineur | `internal/service/campaign_test.go:139` | A2 étape 3 (« invalide pour tout verdict ») et UC-004 A1 (« affiche le statut ») : les messages exigés ne sont pas vérifiés | 0/3 | ouvert | `TestUC003_A2_HarnaisModifiePendantLaCampagne` vérifie les empreintes dans `AbortReason`, pas le message « invalide pour tout verdict ». |
| A-192 | Mineur | `internal/service/campaign_test.go:176` | A3 : le message d'erreur du sujet FAILED n'est pas vérifié dans la mesure écrite | 0/3 | ouvert | `TestUC003_A3_SujetEnEchec` ne lit pas le `FailureReason` de la mesure écrite. |
| A-205 | Mineur | `internal/service/compare.go:172` | Le déterminisme de l'IC bootstrap n'est garanti que pour une même toolchain 64 bits | 0/3 | ouvert | `compare.go` : `seedFor` (FNV) et `rand.NewPCG` inchangés. |
| A-282 | Mineur | `internal/service/criteria.go:55` | evaluateH001 restreint le corpus à ARRAY_FILL, filtre absent du critère gelé de H-001 | 0/3 | ouvert | `evaluateH001` filtre toujours `ARRAY_FILL`, justifié en commentaire, absent du critère gelé. |
| A-283 | Mineur | `internal/service/criteria.go:98` | evaluateH002 ne lit que les séries ARRAY_FILL alors que le critère gelé parle des « deux séries (avec, sans champ pointeur) » sans disposition | 0/3 | ouvert | `evaluateH002` ne lit toujours que les clés `ARRAY_FILL`, même justification. |
| A-206 | Mineur | `internal/service/criteria.go:256` | Les rationales arrondissent les rapports à une précision qui peut contredire le seuil appliqué | 0/3 | ouvert | Rationales toujours formatés en `%.1f` et `%.2f` (`criteria.go`, `criteria_c008.go`). |
| A-255 | Mineur | `internal/service/escape.go:72` | UC-002 étape 2 : la Provenance « qu'il va consigner » n'est affichée qu'à l'étape 7, et jamais en cas d'échec | 0/3 | ouvert | `runEscape` n'imprime la Provenance qu'après `Classify`, et jamais sur échec. |
| A-256 | Mineur | `internal/service/escape.go:136` | Un fichier de verdicts antérieur illisible bloque définitivement UC-002 pour cette matrice | 0/3 | implanté au fil de l'eau | `service/escape.go` et `cli/render.go` (commentaires A-256, commit `6b73dc7`). |
| A-250 | Mineur | `internal/service/matrix.go:112` | Échec de WriteSources ou de Finalize : aucun Remove, le répertoire partiel subsiste | 0/3 | implanté au fil de l'eau | `service/matrix.go` (commentaire A-250) et `TestUC001_RepertoirePartielNettoye` (commit `6b73dc7`). |
| A-249 | Mineur | `internal/service/matrix.go:168` | compileAll traite toute erreur de Build comme un sujet non compilable (A3), sans distinguer *ports.CompileError | 0/3 | implanté au fil de l'eau | `service/matrix.go` (commentaire A-249) et `TestUC001_PanneDeCompilationNestPasUnSujetFautif` (commit `6b73dc7`). |
| A-160 | Mineur | `internal/service/verdict.go:211` | --escape est enregistré tel quel dans resultFiles : chemin absolu ou à barres obliques inverses écrit dans results/verdicts | 0/3 | ouvert | `gather` retient `escapePath` tel que donné, et il passe tel quel dans les fichiers cités par H-006 et H-009. |
| A-191 | Mineur | `internal/service/verdict_test.go:230` | UC-005 A2 : « liste des sujets manquants » ni produite (H-004, H-005, H-011) ni exigée par les tests | 0/3 | ouvert | En partie : H-004 nomme désormais ses sondes manquantes ; H-005 et H-011 rendent encore « sondes d'append … incomplètes » sans liste. |
| A-199 | Suggestion | `cmd/escapebench/main_test.go:130` | La chaîne complète exerce `dashboard` sur une racine sans go.mod : colonnes Unit/Regression jamais vérifiées de bout en bout | 0/3 | ouvert | `main_test.go` : la chaîne complète cherche trois chaînes dans le tableau de bord, pas les colonnes Unit et Regression. |
| A-212 | Suggestion | `internal/adapters/gotool/gotool.go:203` | La fenêtre d'attestation de quiétude englobe la compilation et l'édition de liens, ce qui dilue une contention pendant le seul banc | 0/3 | ouvert | `gotool.go`, `Run` : les deux relevés de quiétude encadrent tout `go test`, compilation et édition de liens comprises. |
| A-200 | Suggestion | `internal/adapters/gotool/gotool_test.go:139` | BR-003-4 « un sujet, un processus » : TestRunMesureUnSujet n'assert pas un seul appel au runner | 0/3 | ouvert | `TestRunMesureUnSujet` vérifie les drapeaux de la dernière commande, pas le nombre d'appels. |
| A-276 | Suggestion | `internal/adapters/store/dto.go:326` | Une Measurement FAILED sérialise ses listes en `null` alors que le modèle d'entités dit « listes vides » | 0/3 | ouvert | `measurementDTO` sérialise `nsPerOp`, `bytesPerOp` et `allocsPerOp` sans normalisation : `null` pour une mesure FAILED. |
| A-234 | Suggestion | `internal/adapters/store/store.go:502` | Le comparateur de TippingKey est recopié trois fois (store.sortedTippingKeys, service/compare.sortedKeys, cli.RenderComparison) et le libellé de… | 0/3 | ouvert | Trois tris de `TippingKey` subsistent : `store.sortedTippingKeys`, `service.sortedKeys`, `cli/render.go`. |
| A-182 | Suggestion | `internal/adapters/store/store.go:558` | os.Rename par-dessus campaign.json sous OneDrive/antivirus peut échouer transitoirement (ERROR_SHARING_VIOLATION) ; aucune reprise | 0/3 | ouvert | `writeFileAtomic` : `os.Rename` sans nouvelle tentative. |
| A-203 | Suggestion | `internal/adapters/store/store_test.go:193` | time.Now() dans TestEscapeReportRoundTripEtImmutabilite | 0/3 | ouvert | `store_test.go` appelle encore `time.Now()`. |
| A-179 | Suggestion | `internal/adapters/system/topology_other.go:7` | darwin et freebsd tombent dans _other : H-008 et H-013 sont non concluantes par construction sur l'arm64 le plus accessible (Apple Silicon), que… | 0/3 | ouvert | `topology_other.go` (`!windows && !linux`) couvre toujours darwin et freebsd. |
| A-178 | Suggestion | `internal/adapters/system/topology_windows.go:25` | Les parseurs purs (parseWindowsCaches, cpuTimesFrom, parseProcStat, parseLinuxCacheSize, readLinuxCaches) et leurs tests ne se compilent que sur leur… | 0/3 | ouvert | Parseurs et tests toujours dans des fichiers `_linux.go` et `_windows.go` (`quietude_parse_linux_test.go`, etc.). |
| A-180 | Suggestion | `internal/harness/harness.go:24` | Aucun test ne garantit que les gabarits embarqués sont en LF : un fichier converti en CRLF par un éditeur Windows change harnessDigest sans que rien… | 0/3 | ouvert | Aucun test de fin de ligne des gabarits ; `.gitattributes` (`eol=lf`, antérieur à l'audit) ne protège pas d'un éditeur qui convertit la copie de travail. |
| A-225 | Suggestion | `internal/models/matrix.go:521` | Matrix.Probe est morte hors tests | 0/3 | ouvert | `Matrix.Probe` n'a toujours d'appelant que dans `models/matrix_test.go`. |
| A-238 | Suggestion | `internal/models/models.go:252` | Repetitions, Payloads, ReplicateIndex et les trois bornages de cellDTO.toModel réécrivent max(1, x) | 0/3 | ouvert | `Repetitions`, `Payloads` et `ReplicateIndex` ramènent toujours à 1 à la main. |
| A-224 | Suggestion | `internal/models/models.go:269` | Cell.ReplicateIndex est morte hors tests | 0/3 | ouvert | `Cell.ReplicateIndex` n'a d'appelant que dans `models/replicat_test.go`. |
| A-260 | Suggestion | `internal/models/models.go:290` | Cell.Validate accepte repeat, payload et replicate à 0 alors que le modèle les déclare « Requis, ≥ 1 » | 0/3 | implanté au fil de l'eau | Traité autrement que proposé : `Cell.Validate` refuse les valeurs négatives et admet zéro, ramené à 1 (commentaire « A-260 écarté, vérification faite », commit `3670986`). |
| A-241 | Suggestion | `internal/models/models.go:661` | Comparison.EffectiveLayout double le défaut déjà appliqué par comparisonSetDTO.toModel ; usage incohérent entre évaluateurs | 0/3 | ouvert | `Comparison.EffectiveLayout` a encore cinq appelants hors tests. |
| A-204 | Suggestion | `internal/models/models_test.go:353` | TestMedians ne couvre pas MedianInt sur un effectif pair | 0/3 | ouvert | `TestMedians` n'exerce `MedianInt` que sur `nil`. |
| A-138 | Suggestion | `internal/models/revue_test.go:19` | Huit fichiers de test sans aucun t.Run : la règle « tests table-driven » de CLAUDE.md n'y est pas appliquée | 0/3 | ouvert | `models/revue_test.go` : aucun `t.Run`. |
| A-240 | Suggestion | `internal/service/campaign.go:122` | Boucle de recherche de « H-007 » remplaçable par slices.Contains | 0/3 | implanté au fil de l'eau | `campaign.go` : `slices.Contains(hypothesisIDs, "H-007")` (commit `3bd223b`). |
| A-274 | Suggestion | `internal/service/campaign.go:187` | CampaignReport.Status est vide sur les chemins d'erreur (verrou refusé, runner en panne) : le rendu affiche « statut : » alors que la campagne est… | 0/3 | implanté au fil de l'eau | Par deux autres correctifs : `runCampaign` n'imprime plus un rapport sans identifiant (A-084) et le verrou est pris avant la création de la campagne (A-263) ; le rendu « statut : » vide ne peut plus se produire. |
| A-134 | Suggestion | `internal/service/campaign.go:250` | Deux erreurs de service sans sentinelle : « aucun sujet mesuré » (campaign.go:250) et « n sujet(s) ne compilent pas » (matrix.go:122) | 2/3 | implanté au fil de l'eau | Sentinelles `ErrNoSubjectMeasured` et `ErrSubjectsNotCompilable` (`service/matrix.go`, commit `9e81f6d`). |
| A-201 | Suggestion | `internal/service/campaign_test.go:393` | BR-003-2 (Provenance complète obligatoire) sans test au niveau du service | 0/3 | ouvert | Aucun test de service ne soumet une Provenance incomplète (tests `TestUC003_*` recensés). |
| A-290 | Suggestion | `internal/service/compare.go:96` | Quand toutes les paires sont exclues, les raisons d'exclusion ne sont consignées nulle part | 0/3 | ouvert | `Compare` rend `ErrPrecondition` sans écrire les exclusions quand aucune paire n'est complète. |
| A-202 | Suggestion | `internal/service/compare_test.go:140` | UC-004 A3 n'a pas de test nommé TestUC004_A3_ ; BR-001-2, BR-002-3, BR-003-3, BR-004-3 n'ont pas de TestUC###_BR#_ | 0/3 | ouvert | Ni `TestUC004_A3_`, ni `TestUC001_BR2`, `TestUC002_BR3`, `TestUC004_BR3` ; BR-003-3 a désormais les siens. |
| A-243 | Suggestion | `internal/service/criteria.go:31` | service.SmallStructBytes est un alias exporté de models.SmallStructBytes que personne n'importe | 0/3 | implanté au fil de l'eau | Alias retiré (commentaire A-243, `criteria.go`, commit `3bd223b`). |
| A-210 | Suggestion | `internal/service/criteria.go:68` | H-001 compte des Comparison, non des tailles distinctes, contrairement au texte gelé « au moins deux tailles » | 0/3 | ouvert | `evaluateH001` compte toujours des Comparison (`len(refuting) >= 2`). |
| A-230 | Suggestion | `internal/service/criteria_c008.go:115` | pointerWins a un paramètre ref jamais différent de c | 0/3 | implanté au fil de l'eau | Paramètre `ref` retiré de `pointerWins` (commentaire A-230, commit `3bd223b`). |
| A-245 | Suggestion | `internal/service/criteria_c008.go:223` | evaluateH009 appelle Matrix.Cell (balayage linéaire avec reconstruction de l'ID) dans deux boucles imbriquées sur les verdicts | 0/3 | ouvert | `evaluateH009` appelle toujours `e.Matrix.Cell` dans ses deux boucles. |
| A-228 | Suggestion | `internal/service/criteria_c008.go:416` | abs réinvente math.Abs | 0/3 | implanté au fil de l'eau | Plus aucune fonction `abs` dans le module (commit `3bd223b`). |
| A-233 | Suggestion | `internal/service/criteria_c009.go:22` | h012Cell.medianValue, medianPointer et significant ne servent qu'à l'intérieur de h012CellsOf | 0/3 | ouvert | `h012Cell` porte toujours `medianValue`, `medianPointer` et `significant`. |
| A-232 | Suggestion | `internal/service/criteria_c009.go:42` | h012JudgedCells rend un struct anonyme alors que h012Key porte exactement les mêmes champs | 0/3 | ouvert | `h012JudgedCells` rend toujours un struct anonyme. |
| A-242 | Suggestion | `internal/service/criteria_c009.go:60` | H-012 code en dur deux identifiants de campagne là où H-011 a une constante nommée PreH011CampaignID | 0/3 | ouvert | `criteria_c009.go` code toujours en dur `"C-2026-09-10-1", "C-2026-09-10-2"`. |
| A-231 | Suggestion | `internal/service/criteria_c009.go:156` | replicateCount est redondante : la valeur zéro de h012Cell a déjà len(deltas) == 0 | 0/3 | ouvert | `replicateCount` existe toujours. |
| A-227 | Suggestion | `internal/service/criteria_c009.go:230` | medianOf recopie models.MedianFloat à l'identique | 0/3 | implanté au fil de l'eau | Plus de `medianOf` ; `models.MedianFloat` sert (commit `3bd223b`). |
| A-229 | Suggestion | `internal/service/criteria_c009.go:244` | maxOf réinvente le builtin max (Go 1.21+, module en go 1.25) | 0/3 | implanté au fil de l'eau | Plus de `maxOf` ; le `max` intégré sert (commit `3bd223b`). |
| A-257 | Suggestion | `internal/service/escape.go:162` | DifferingVerdicts ignore compilerReason et compilerError alors que NFR-002 exige des verdicts identiques cellule par cellule | 0/3 | ouvert | `DifferingVerdicts` compare toujours `Escapes`, `Category` et `Status` seulement. |
| A-198 | Suggestion | `internal/service/fakes_test.go:210` | Le memoryStore n'a jamais de collision d'horodatage, le store réel si (résolution 1 s) | 0/3 | ouvert | `memoryStore.nextStamp` ne produit jamais de collision. |
| A-286 | Suggestion | `internal/service/verdict.go:226` | Dashboard lance `go test ./...` six fois (RunAll + un RunUseCaseTests par UC) alors qu'une seule exécution -v suffit | 0/3 | ouvert | `verdict.go` : `RunAll`, puis `RunUseCaseTests` par UC. |
| A-244 | Suggestion | `internal/service/verdict.go:319` | join est un alias trivial de strings.Join(values, ", ") et n'est pas utilisé uniformément | 0/3 | ouvert | `join` existe toujours (`verdict.go`). |
| A-239 | Suggestion | `internal/service/verdict.go:322` | sortedSubjectIDs et store.sortedKeys réinventent slices.Sorted(maps.Keys(m)) | 0/3 | ouvert | En partie : `sortedSubjectIDs` passe à `slices.Sorted(maps.Keys(m))` ; `store.sortedKeys` trie encore à la main. |
